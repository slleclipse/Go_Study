...
import (
	...
	"gochannel/main/chapter_5/main/cmap"
	"gochannel/main/chapter_60/webcrawler/module"
	"gochannel/main/chapter_60/webcrawler/toolkit/buffer"
	"net/http"
	"strings"
	"sync"
)
...
func (sched *myScheduler) Init(
	...
	moduleArgs ModuleArgs) (err error) {
	// 检查状态。
	logs.Info("Check status for initialization...")
	...
	// 检查参数。
	logs.Info("Check request arguments...")
	if err = requestArgs.Check(); err != nil {
		return err
	}
	logs.Info("Check data arguments...")
	if err = dataArgs.Check(); err != nil {
		return err
	}
	logs.Info("Data arguments are valid.")
	logs.Info("Check module arguments...")
	if err = moduleArgs.Check(); err != nil {
		return err
	}
	logs.Info("Module arguments are valid.")
	// 初始化内部字段。
	logs.Info("Initialize scheduler’s fields...")
	...
	logs.Info("-- Max depth: %d", sched.maxDepth)
	...
	logs.Info("-- Accepted primary domains: %v",
		requestArgs.AcceptedDomains)
	sched.urlMap, _ = cmap.NewConcurrentMap(16, nil)
	logs.Info("-- URL map: length: %d, concurrency: %d",
	...
	// 注册组件。
	logs.Info("Register modules...")
	if err = sched.registerModules(moduleArgs); err != nil {
		return err
	}
	logs.Info("Scheduler has been initialized.")
	return nil
}
...
func (sched *myScheduler) Start(firstHTTPReq *http.Request) (err error) {
	...
	logs.Info("Start scheduler...")
	// 检查状态。
	logs.Info("Check status for start...")
	var oldStatus Status
	oldStatus, err =
	...
	// 检查参数。
	logs.Info("Check first HTTP request...")
	if firstHTTPReq == nil {
		err = genParameterError("nil first HTTP request")
		return
	}
	logs.Info("The first HTTP request is valid.")
	// 获得首次请求的主域名，并将其添加到可接受的主域名的字典。
	logs.Info("Get the primary domain...")
	logs.Info("-- Host: %s", firstHTTPReq.Host)
	var primaryDomain string
	primaryDomain, err = getPrimaryDomain(firstHTTPReq.Host)
	if err != nil {
		return
	}
	logs.Info("-- Primary domain: %s", primaryDomain)
	...
	sched.analyze()
	sched.pick()
	logs.Info("Scheduler has been started.")
	// 放入第一个请求。
...
func (sched *myScheduler) Stop() (err error) {
	logs.Info("Stop scheduler...")
	// 检查状态。
	logs.Info("Check status for stop...")
	var oldStatus Status
	oldStatus, err =
	...
	logs.Info("Scheduler has been stopped.")
	return nil
}
...
func (sched *myScheduler) ErrorChan() <-chan error {
	...
	go func(errBuffer buffer.Pool, errCh chan error) {
		for {
			...
			if err != nil {
				logs.Warn("The error buffer pool was closed. Break error reception.")
				close(errCh)
				break
			}
			err, ok := datum.(error)
...
func (sched *myScheduler) registerModules(moduleArgs ModuleArgs) error {
	...
	logs.Info("All downloads have been registered. (number: %d)",
	...
	logs.Info("All analyzers have been registered. (number: %d)",
	...
	logs.Info("All pipelines have been registered. (number: %d)",
...
func (sched *myScheduler) download() {
	go func() {
		for {
			...
			if err != nil {
				logs.Warn("The request buffer pool was closed. Break request reception.")
				break
			}
...
func (sched *myScheduler) analyze() {
	go func() {
		for {
			...
			if err != nil {
				logs.Warn("The response buffer pool was closed. Break response reception.")
				break
			}
...
func (sched *myScheduler) pick() {
	go func() {
		for {
			...
			if err != nil {
				logs.Warn("The item buffer pool was closed. Break item reception.")
				break
			}
...
func (sched *myScheduler) sendReq(req *module.Request) bool {
	...
	httpReq := req.HTTPReq()
	if httpReq == nil {
		logs.Warn("Ignore the request! Its HTTP request is invalid!")
		return false
	}
	reqURL := httpReq.URL
	if reqURL == nil {
		logs.Warn("Ignore the request! Its URL is invalid!")
		return false
	}
	scheme := strings.ToLower(reqURL.Scheme)
	if scheme != "http" && scheme != "https" {
		logs.Warn("Ignore the request! Its URL scheme is %q, but should be %q or %q. (URL: %s)\n",
			scheme, "http", "https", reqURL)
		return false
	}
	if v := sched.urlMap.Get(reqURL.String()); v != nil {
		logs.Warn("Ignore the request! Its URL is repeated. (URL: %s)\n", reqURL)
		return false
	}
	...
	if sched.acceptedDomainMap.Get(pd) == nil {
		...
		logs.Warn("Ignore the request! Its host %q is not in accepted primary domain map. (URL: %s)\n",
			httpReq.Host, reqURL)
		return false
	}
	if req.Depth() > sched.maxDepth {
		logs.Warn("Ignore the request! Its depth %d is greater than %d. (URL: %s)\n",
			req.Depth(), sched.maxDepth, reqURL)
		return false
	}
	go func(req *module.Request) {
		if err := sched.reqBufferPool.Put(req); err != nil {
			logs.Warn("The request buffer pool was closed. Ignore request sending.")
		}
	}(req)
...
func sendResp(resp *module.Response, respBufferPool buffer.Pool) bool {
	...
	go func(resp *module.Response) {
		if err := respBufferPool.Put(resp); err != nil {
			logs.Warn("The response buffer pool was closed. Ignore response sending.")
		}
	}(resp)
	return true
}
...
func sendItem(item module.Item, itemBufferPool buffer.Pool) bool {
	...
	go func(item module.Item) {
		if err := itemBufferPool.Put(item); err != nil {
			logs.Warn("The item buffer pool was closed. Ignore item sending.")
		}
	}(item)
	return true
}
...
func (sched *myScheduler) initBufferPool(dataArgs DataArgs) {
	...
	logs.Info("-- Request buffer pool: bufferCap: %d, maxBufferNumber: %d",
	...
	logs.Info("-- Response buffer pool: bufferCap: %d, maxBufferNumber: %d",
	...
	logs.Info("-- Item buffer pool: bufferCap: %d, maxBufferNumber: %d",
	...
	logs.Info("-- Error buffer pool: bufferCap: %d, maxBufferNumber: %d",

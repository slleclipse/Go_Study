...
func reqHandler(conn net.Conn) {
	...
	if err != nil {
		logs.Error("Server:　Resp Marshal Error: %s", err)
	}
	_, err = write(conn, bytes, DELIM)
	if err != nil {
		logs.Error("Server: Resp Write error: %s", err)
	}
}
...
func (server *TCPServer) Listen(addr string) error {
	...
	go func() {
		for {
			...
			if err != nil {
				if atomic.LoadUint32(&server.active) == 1 {
					logs.Error("Server: Request Acception Error: %s\n", err)
				} else {
					logs.Error("Server: Broken acception because of closed network connection.")
				}
				continue
			}
			go reqHandler(conn)

...
import (
	...
	"gochannel/main/chapter_4/loadgenerator/lib"
	"math"
	"sync/atomic"
	"time"
)
...
func (gen *myGenerator) Start() bool {
	...
	if !atomic.CompareAndSwapUint32(
		&gen.status, lib.STATUS_ORGINAL, lib.STATUS_STARTING) {
	...
	if gen.lps > 0 {
		...
		logs.Info("Setting throttle (%v)...", interval)
		throttle = time.Tick(interval)
	}

	// 初始化上下文和取消函数
	gen.ctx, gen.cancleFunc = context.WithTimeout(
	...
	go func() {
		...
		logs.Info("Stopped. (call count: %d)", gen.callCount)
	}()
	return true
}
...
func (gen *myGenerator) Stop() bool {
	if !atomic.CompareAndSwapUint32(
		&gen.status, lib.STATUS_STARTED, lib.STATUS_STOPPED) {
		return false
	}
	gen.cancleFunc()
	for {
